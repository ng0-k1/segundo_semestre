class Estudiante():
    ...
    def ingresar(self,notas=1,suma=0,contador=0):
        ...
        cant_notas = int(input('Ingrese la cantidad de notas a digitar: '))
        lista2 = []
        for i in range(0,cant_notas,1):
            self.notas=float(input(f"digite las nota {i+1} del estudiante: "))
            lista2.append(self.notas)
            notas=self.notas
            notas!=1
            suma+=notas
            contador+=1
        self.lista.append([self.codigo, self.alumno,lista2])
        print('\n')
        if contador==0:
        ...
        for e in self.lista:
            reg=[self.codigo,self.alumno,lista2,self.promedio]
    ...
    def nota(self):
        ...
        if p<0:
            ...
            print(f'las notas del estudiante son: {str(self.datos[p][2])[1:-1]} ') 
    def promedios(self):
        c=int(input("ingrese el código del estudiante: "))
        p=-1;
        for i in range(len(self.datos)):
        ...
        if p<0:
            ...
            print(f"Promedio estudiante : {self.datos[p][3]}")
    def menu(self):
        estado=0
        while True:
            print()
